<script setup lang="ts">
  ...
  const props = defineProps<{
    ...
    dotKinds: DotKind[] // 固定 4 格，每格自帶色碼 + 相對權重
  }>()
  ...
  // 【圖層：點點】兩階段隨機：先依 density 決定這格要不要出現點點，再依 dotKinds 的權重加權抽色碼
  // （權重不需正規化，抽色時除以權重總和即可；權重全 0 則該格不畫點）。
  // 只依 rows/cols/density/dotKinds 重新隨機，改 cellSize 只換算像素位置，
  // 故拆成 dotCells（邏輯格 + 色碼）→ dots（像素座標 + 色碼）兩層 computed。
  const dotCells = computed<{ r: number; c: number; color: string }[]>(() => {
    const total = props.dotKinds.reduce((sum, kind) => sum + kind.weight, 0)
    ...
    for (let r = 0; r < props.rows; r++) {
      for (let c = 0; c < props.cols; c++) {
        ...
        let color = props.dotKinds[0]!.color
        for (const kind of props.dotKinds) {
          pick -= kind.weight
          if (pick <= 0) {
            color = kind.color
            break
...
<template>
  <svg :width="dims.w + STROKE" :height="dims.h + STROKE" :viewBox="viewBox" class="text-gray-800 dark:text-gray-200">
    ...
    <!-- 圖層 2：點點（四色共用一個圖層） -->
    <g class="layer-dots">
      <circle v-for="(d, i) in dots" :key="i" :cx="d.cx" :cy="d.cy" :r="cellSize * 0.28" :fill="d.color" />
    </g>
  </svg>
</template>
